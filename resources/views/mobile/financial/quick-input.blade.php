...
<div x-data="financialInput()" class="pb-20">
    {{-- Type Toggle --}}
    <div class="sticky top-16 bg-white z-10 px-4 pt-4 pb-3 border-b border-gray-100">
        <div class="inline-flex bg-gray-100 rounded-xl p-1 w-full">
            <button 
                @click="type = 'income'" 
                :class="type === 'income' ? 'bg-green-600 text-white shadow-md' : 'text-gray-600'" 
                class="flex-1 py-3 rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2">
                <i class="fas fa-arrow-down"></i>
                <span>Uang Masuk</span>
            </button>
            <button 
                @click="type = 'expense'" 
                :class="type === 'expense' ? 'bg-red-600 text-white shadow-md' : 'text-gray-600'" 
                class="flex-1 py-3 rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2">
    ...
    <form @submit.prevent="submitTransaction" class="p-4 space-y-4">
        {{-- Amount Input --}}
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Jumlah</label>
            <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-xl">Rp</span>
                <input 
                    type="text" 
                    x-model="amount" 
                    @input="formatAmount"
                    inputmode="numeric"
                    placeholder="0"
                    class="w-full pl-14 pr-4 py-4 text-2xl font-bold border-2 border-gray-200 rounded-xl 
        ...
        {{-- Submit Button --}}
        <div class="pt-4 sticky bottom-0 bg-white pb-4 -mx-4 px-4 border-t border-gray-100">
            <button 
                type="submit"
                :disabled="loading || !amount || !category"
                :class="type === 'income' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'"
                class="w-full py-4 text-white rounded-xl font-bold text-lg 
