...
<div class="pb-20" x-data="dashboardMobile()">
    ...
    <div class="grid grid-cols-2 gap-2 mb-4">
        ...
        <div onclick="window.location.href='{{ mobile_route('approvals.index') }}'"
             class="bg-white border-2 border-gray-200 rounded-lg p-3 shadow-sm active:scale-95 transition-transform cursor-pointer">
            <div class="flex items-center gap-2 mb-2">
                <div class="w-8 h-8 bg-[#E7F3F8] rounded-lg flex items-center justify-center">
                    <i class="fas fa-file-signature text-[#0A66C2] text-sm"></i>
                </div>
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between mb-3">
            <h2 class="text-lg font-bold text-gray-900">
                <i class="fas fa-calendar-day text-[#0A66C2] mr-2"></i>
                Agenda Hari Ini
            </h2>
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden" x-data="{ expanded: false }">
            <button @click="expanded = !expanded" 
                ...
                <div class="flex items-center gap-2">
                    <i class="fas fa-folder-open text-[#0A66C2]"></i>
                    <span class="font-semibold text-gray-900">Proyek Aktif</span>
                    <span class="text-xs px-2 py-1 bg-[#E7F3F8] text-[#004182] rounded-full font-medium">
                        {{ $projectStats['active'] }}
                    </span>
                </div>
            ...
            <div x-show="expanded" 
                ...
                <a href="{{ mobile_route('projects.index') }}" 
                   class="block py-2 text-sm text-[#0A66C2] font-medium">
                    Lihat semua proyek →
                </a>
            </div>
        </div>
    ...
    <button x-show="showBackToTop" 
            @click="scrollToTop()"
            x-transition
            class="fixed bottom-24 right-4 w-12 h-12 bg-[#0A66C2] text-white rounded-full shadow-lg 
                   flex items-center justify-center hover:bg-[#004182] transition-colors z-40">
    ...
    return {
        ...
        showToast(message, type) {
            ...
            toast.className = `fixed top-4 right-4 px-4 py-3 rounded-lg shadow-lg z-50 
                              ${type === 'success' ? 'bg-green-500' : 'bg-[#0A66C2]'} text-white`;
