...
@php
    ...
    $statusStyle = $interview->status === 'completed'
        ? 'background: var(--neuro-success); opacity: var(--opacity-bg-light); color: var(--neuro-success);'
        : ($interview->status === 'scheduled'
            ? 'background: var(--neuro-primary); opacity: var(--opacity-bg-light); color: var(--neuro-primary);'
            : 'background: var(--text-dark-tertiary); opacity: var(--opacity-bg-light); color: var(--text-dark-tertiary);');
@endphp
...
<div class="recruitment-shell max-w-6xl mx-auto space-y-5">
    ...
    <section class="card-elevated rounded-apple-xl p-5 md:p-6 relative overflow-hidden">
        <div class="absolute inset-0 pointer-events-none" aria-hidden="true">
            <div class="w-72 h-72 blur-3xl rounded-full absolute -top-14 -right-10" style="background: var(--neuro-primary); opacity: var(--opacity-bg-light);"></div>
            <div class="w-48 h-48 blur-2xl rounded-full absolute bottom-0 left-8" style="background: var(--neuro-success); opacity: var(--opacity-bg-light);"></div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="card-elevated rounded-apple-xl p-4 space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs uppercase tracking-[0.3em]" style="color: var(--text-dark-tertiary); opacity: var(--opacity-text-light);">Informasi</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm" style="color: var(--text-dark-secondary); opacity: var(--opacity-text-strong);">
                    <div class="space-y-1">
                        <p class="text-xs uppercase tracking-widest" style="color: var(--text-dark-tertiary); opacity: var(--opacity-text-light);">Kandidat</p>
                        <p class="font-semibold text-white">{{ $application->full_name }}</p>
                        <p class="text-xs" style="color: var(--text-dark-secondary); opacity: var(--opacity-text-strong);">{{ $application->email }}{{ $application->phone ? ' · ' . $application->phone : '' }}</p>
                    </div>
                    <div class="space-y-1">
                        <p class="text-xs uppercase tracking-widest" style="color: var(--text-dark-tertiary); opacity: var(--opacity-text-light);">Posisi</p>
                        <p class="font-semibold text-white">{{ $vacancy->title }}</p>
                        <p class="text-xs" style="color: var(--text-dark-secondary); opacity: var(--opacity-text-strong);">{{ $vacancy->location }}</p>
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm" style="color: var(--text-dark-secondary); opacity: var(--opacity-text-strong);">
                    <div class="space-y-1">
                        <p class="text-xs uppercase tracking-widest" style="color: var(--text-dark-tertiary); opacity: var(--opacity-text-light);">Jadwal</p>
                        <p class="font-semibold text-white">{{ $interview->scheduled_at->format('d M Y, H:i') }} WIB</p>
                        <p class="text-xs" style="color: var(--text-dark-secondary); opacity: var(--opacity-text-strong);">{{ $interview->scheduled_at->diffForHumans() }}</p>
                    </div>
                    <div class="space-y-1">
                        <p class="text-xs uppercase tracking-widest" style="color: var(--text-dark-tertiary); opacity: var(--opacity-text-light);">Pertemuan</p>
                        ...
                        @elseif($interview->location)
                            <p class="text-xs" style="color: var(--text-dark-secondary); opacity: var(--opacity-text-strong);">{{ $interview->location }}</p>
                        @endif
                    </div>
                </div>
