...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="card-elevated rounded-apple-lg p-4 space-y-2">
            <p class="text-xs uppercase tracking-widest text-dark-text-secondary">Total Jenis</p>
            <p class="text-xl font-bold text-white">{{ $totalTypes }}</p>
            <p class="text-xs text-dark-text-secondary">Izin terdaftar</p>
        </div>
        <div class="card-elevated rounded-apple-lg p-4 space-y-2">
            <p class="text-xs uppercase tracking-widest text-apple-green/90">Aktif</p>
            <p class="text-xl font-bold text-apple-green">{{ $activeTypes }}</p>
            <p class="text-xs text-dark-text-secondary">Bisa digunakan klien</p>
        </div>
        <div class="card-elevated rounded-apple-lg p-4 space-y-2">
            <p class="text-xs uppercase tracking-widest text-apple-blue/90">Total Permohonan</p>
            <p class="text-xl font-bold text-apple-blue">{{ $totalApplications }}</p>
            <p class="text-xs text-dark-text-secondary">Menggunakan jenis ini</p>
        </div>
        <div class="card-elevated rounded-apple-lg p-4 space-y-2">
            <p class="text-xs uppercase tracking-widest text-apple-orange/90">Harga Rata-rata</p>
            <p class="text-xl font-bold text-apple-orange">
                {{ $avgPrice ? 'Rp '.number_format($avgPrice/1000, 0).'K' : 'N/A' }}
            </p>
            <p class="text-xs text-dark-text-secondary">Base price</p>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between gap-3">
        <div>
            <p class="text-xs uppercase tracking-[0.35em] text-dark-text-tertiary">Manajemen</p>
                    ...
                    @forelse($permitTypes as $type)
                        <tr class="hover-lift transition-apple">
                            ...
                            <td class="px-4 py-2.5 text-center whitespace-nowrap">
                                ...
                                @else
                                    <span class="px-2 py-1 text-xs font-medium rounded-apple bg-dark-surface-tertiary text-dark-text-tertiary">
                                        Nonaktif
                                    </span>
                                @endif
                            </td>
                            <td class="px-4 py-2.5 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center space-x-1.5">
                                    ...
                                    <a href="{{ route('permit-types.edit', $type) }}"
                                       class="inline-flex items-center px-2.5 py-1 rounded-apple text-xs font-semibold transition-apple bg-apple-orange/20 text-apple-orange border border-apple-orange/30">
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    <i class="fas fa-certificate text-4xl mb-6 text-dark-text-tertiary"></i>
                                    <h3 class="text-base font-semibold mb-2 text-white">Belum Ada Jenis Izin</h3>
