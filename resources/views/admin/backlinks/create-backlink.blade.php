...
@section('title', 'Add Backlink')
@section('page-title', 'Add New Backlink')
...
<div style="max-width: 900px; margin: 0 auto;">
    ...
    <div style="margin-bottom: 2rem;">
        ...
        <h1 style="font-size: 1.75rem; font-weight: 700; color: var(--dark-text-primary); margin: 0;">
            <i class="fas fa-plus-circle" style="color: var(--apple-green); margin-right: 0.75rem;"></i>
            Add New Backlink
        </h1>
        <p style="font-size: 0.875rem; color: var(--dark-text-secondary); margin: 0.5rem 0 0 0;">
            Tambahkan backlink yang berhasil didapatkan untuk tracking
        </p>
    </div>

    <!-- Form Card -->
    <div class="card-elevated rounded-apple-xl" style="background: var(--dark-bg-elevated); padding: 2rem;">
        <form method="POST" action="{{ route('backlinks.store') }}">
            @csrf
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select name="target_id" required 
                    ...
                    @foreach($targets as $target)
                        <option value="{{ $target->id }}">{{ $target->website_name }} (DA: {{ $target->domain_authority }})</option>
                    @endforeach
                </select>
                @error('target_id')
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="url" name="source_url" required value="{{ old('source_url') }}"
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="url" name="target_url" required value="{{ old('target_url') }}"
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem; margin-bottom: 1.5rem;">
                ...
                <div>
                    ...
                    <select name="type" required 
                            ...
                            onfocus="this.style.borderColor='var(--apple-blue)'; this.style.boxShadow='0 0 0 3px rgba(0, 122, 255, 0.1)'"
                            onblur="this.style.borderColor='rgba(84, 84, 88, 0.3)'; this.style.boxShadow='none'">
                        <option value="dofollow" {{ old('type') == 'dofollow' ? 'selected' : '' }}>DoFollow</option>
                        <option value="nofollow" {{ old('type') == 'nofollow' ? 'selected' : '' }}>NoFollow</option>
                    </select>
                    @error('type')
                        <p style="color: var(--apple-red); font-size: 0.75rem; margin: 0.5rem 0 0 0;">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" required 
                        ...
                        <option value="indexed" {{ old('status') == 'indexed' ? 'selected' : '' }}>Indexed</option>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem; margin-bottom: 1.5rem;">
                ...
                <div>
                    ...
                    <input type="date" name="acquired_at" value="{{ old('acquired_at', now()->format('Y-m-d')) }}"
                           style="width: 100%; padding: 0.875rem; background: var(--dark-bg-tertiary); border: 1px solid rgba(84, 84, 88, 0.3); border-radius: 10px; color: var(--dark-text-primary); font-size: 0.875rem; transition: all 0.2s ease;"
                           onfocus="this.style.borderColor='var(--apple-blue)'; this.style.boxShadow='0 0 0 3px rgba(0, 122, 255, 0.1)'"
                           onblur="this.style.borderColor='rgba(84, 84, 88, 0.3)'; this.style.boxShadow='none'">
                    @error('acquired_at')
            ...
            <div style="margin-bottom: 2rem;">
                ...
                <textarea name="notes" rows="4"
                          ...
                          onfocus="this.style.borderColor='var(--apple-blue)'; this.style.boxShadow='0 0 0 3px rgba(0, 122, 255, 0.1)'"
                          onblur="this.style.borderColor='rgba(84, 84, 88, 0.3)'; this.style.boxShadow='none'">{{ old('notes') }}</textarea>
                @error('notes')
                    <p style="color: var(--apple-red); font-size: 0.75rem; margin: 0.5rem 0 0 0;">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div style="display: flex; gap: 1rem; justify-content: flex-end; padding-top: 1.5rem; border-top: 1px solid var(--dark-separator);">
                ...
                <button type="submit" 
                        style="padding: 0.75rem 1.5rem; background: linear-gradient(135deg, var(--apple-green) 0%, #28a745 100%); color: #FFFFFF; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; box-shadow: 0 4px 12px rgba(52, 199, 89, 0.3); transition: all 0.3s ease;"
                        onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 6px 16px rgba(52, 199, 89, 0.4)'"
                        onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 12px rgba(52, 199, 89, 0.3)'">
