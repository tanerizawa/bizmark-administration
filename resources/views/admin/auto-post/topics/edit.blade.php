...
<div class="container-fluid px-4 py-6">
    <div class="max-w-3xl mx-auto">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Topic</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                Update topic untuk artikel auto-generated
            </p>
        </div>

        <form action="{{ route('auto-post.topics.update', $topic) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                ...
                <textarea 
                    ...
                    class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-blue-500 focus:ring-blue-500">{{ old('description', $topic->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror>
            </div>
