<div class="space-y-1">
    <!-- Hero -->
    <div class="client-hero overflow-hidden">
        <div class="px-4 lg:px-6 py-6 lg:py-8">
            <div class="flex items-start justify-between gap-6">
                ...
                <div class="text-right space-y-1">
                    ...
                    @else
                    <p class="text-xs text-amber-300"><i class="fas fa-exclamation-triangle"></i> Email belum diverifikasi</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    @if(!$client->email_verified_at)
    <div class="bg-amber-50 dark:bg-amber-900/20 border-y border-amber-200 dark:border-amber-800 px-4 lg:px-6 py-3">
        <div class="flex items-center gap-3 text-amber-800 dark:text-amber-200">
            ...
            <p class="text-sm flex-1"><strong>Email belum diverifikasi.</strong> Verifikasi email untuk mengamankan akun dan menerima notifikasi penting.</p>
            <form method="POST" action="{{ route('client.verification.send') }}">
                @csrf
                <button type="submit" class="flex-shrink-0 text-xs font-semibold px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl active:scale-95 transition">
        ...
        <div class="bg-white dark:bg-gray-900 border-y border-gray-200 dark:border-gray-700">
            ...
            <form method="POST" action="{{ route('client.profile.update') }}" enctype="multipart/form-data" class="px-4 lg:px-6 py-6 space-y-5">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-shrink-0">
                            <div class="w-20 h-20 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700">
                                ...
                                <img id="profilePreview" src="{{ asset('storage/' . $client->profile_picture) }}" alt="Profile" loading="lazy" class="w-full h-full object-cover">
                                @else
                    ...
                    @if($client->profile_picture)
                    <div class="mt-2" x-data="{ confirmDelete: false }">
                        <button
                            type="button"
                            @click="confirmDelete = true"
                            class="text-sm text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 active:scale-95 transition-all"
                        >
                        ...
                        <div x-show="confirmDelete" x-cloak
                            ...
                            <div class="bg-white dark:bg-gray-900 rounded-xl p-6 max-w-sm w-full shadow-xl">
                                <div class="flex items-center gap-3 mb-3">
                                    ...
                                    <h3 class="font-semibold text-gray-900 dark:text-white">Hapus Foto Profil?</h3>
                                </div>
                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-5">Foto profil Anda akan dihapus secara permanen.</p>
                                <div class="flex gap-3">
                                    <button type="button" @click="confirmDelete = false"
                                            class="flex-1 py-2.5 px-4 border border-gray-300 dark:border-gray-600 text-sm font-medium text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition active:scale-95">
                                        Batal
                                    </button>
                                    <button type="button" @click="$refs.deletePhotoForm.submit()"
                                            class="flex-1 py-2.5 px-4 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-lg transition active:scale-95">
                                        Ya, Hapus
                                    </button>
                                </div>
                            </div>
                        </div>
                        <form x-ref="deletePhotoForm" method="POST" action="{{ route('client.profile.update') }}" class="hidden">
                            @csrf
                            @method('PUT')
    ...
    <div class="bg-white dark:bg-gray-900 border-y border-gray-200 dark:border-gray-700"
         x-data="{
             prefs: {
                 email_permits: {{ $client->notification_preferences['email_permits'] ?? 'true' }},
                 email_payments: {{ $client->notification_preferences['email_payments'] ?? 'true' }},
                 push_alerts: {{ $client->notification_preferences['push_alerts'] ?? 'false' }},
                 wa_updates: {{ $client->notification_preferences['wa_updates'] ?? 'false' }},
             },
             saving: false,
             saved: false,
             async save() {
                 this.saving = true;
                 await window.apiFetch('{{ route('client.profile.notifications') }}', {
                     method: 'POST',
        ...
        <div class="px-4 lg:px-6 py-6 border-b border-gray-200 dark:border-gray-700 flex items-center gap-3">
            <i class="fas fa-bell text-[#0a66c2] text-xl"></i>
            <div class="flex-1">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Preferensi Notifikasi</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">Pilih cara Anda ingin diberitahu.</p>
            </div>
            <button type="button" @click="save()"
                    :disabled="saving"
                    :class="saved ? 'bg-green-500' : 'bg-[#0a66c2] hover:bg-[#004182]'"
                    class="text-xs font-semibold px-3 py-1.5 text-white rounded-xl transition active:scale-95">
                <i class="fas" :class="saved ? 'fa-check' : (saving ? 'fa-spinner fa-spin' : 'fa-save')"></i>
                <span x-text="saved ? 'Tersimpan' : (saving ? 'Menyimpan…' : 'Simpan')"></span>
            </button>
        </div>
        <div class="px-4 lg:px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
            @foreach([
                ['email_permits',  'fa-envelope',      'Email — Status Perizinan',    'Notifikasi perubahan status permohonan via email'],
                ['email_payments', 'fa-credit-card',   'Email — Pembayaran',          'Konfirmasi & pengingat pembayaran via email'],
                ['push_alerts',    'fa-bell',           'Push Notification',           'Notifikasi browser untuk update real-time'],
                ['wa_updates',     'fab fa-whatsapp',   'WhatsApp Updates',            'Notifikasi ke nomor WhatsApp terdaftar'],
            ] as [$key, $icon, $label, $desc])
            <label class="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-xl cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700/50 transition">
                <div class="mt-0.5 w-8 h-8 rounded-lg bg-[#0a66c2]/10 flex items-center justify-center flex-shrink-0">
                    <i class="{{ $icon }} text-[#0a66c2] text-sm"></i>
                </div>
                ...
                <div class="flex-shrink-0 mt-0.5">
                    <button type="button" role="switch" :aria-checked="prefs.{{ $key }}"
                            @click="prefs.{{ $key }} = !prefs.{{ $key }}"
                            :class="prefs.{{ $key }} ? 'bg-[#0a66c2]' : 'bg-gray-300 dark:bg-gray-600'"
                            class="relative inline-flex h-5 w-9 rounded-full transition-colors duration-200">
                        <span :class="prefs.{{ $key }} ? 'translate-x-4' : 'translate-x-0.5'"
                              class="inline-block h-4 w-4 mt-0.5 rounded-full bg-white shadow transition-transform duration-200"></span>
                    </button>
                </div>
            </label>
            @endforeach
