...
class PermitManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // FIX (BUG-05): Only load data for the active tab instead of all 5 tabs.
        // Previously all tabs preloaded every request (~15+ queries), now only the
        // active tab's data is fetched (~3-5 queries), reducing load by ~70%.
     ...
     *
     * FIX (BUG-06): Capped at MAX_VARIANTS to prevent SQL query bloat.
     * Previously, for a search term matching many mappings, the query would
     * generate 3N OR conditions (code/description/activities per variant),
     * potentially reaching 150+ OR clauses which killed query performance.
     */
