...
namespace App\Modules\Proyek\Controllers\Mobile;
...
class ProjectController extends Controller
{
        ...
        if ($search) {
            ...
            $query->where(function ($q) use ($searchLower) {
                $q->whereRaw('LOWER(name) LIKE ?', ["%{$searchLower}%"])
                    ...
                    ->orWhereHas('institution', fn ($iq) => $iq->whereRaw('LOWER(name) LIKE ?', ["%{$searchLower}%"])
                    );
            });
        }
    ...
    public function search(Request $request)
    {
        ...
        $projects = Project::whereRaw('LOWER(name) LIKE ?', ["%{$searchLower}%"])
            ->orWhereRaw('LOWER(project_code) LIKE ?', ["%{$searchLower}%"])
            ->orWhereHas('institution', fn ($q) => $q->whereRaw('LOWER(name) LIKE ?', ["%{$searchLower}%"])
            )
            ->orWhereHas('client', fn ($q) => $q->whereRaw('LOWER(company_name) LIKE ?', ["%{$searchLower}%"])
            )
