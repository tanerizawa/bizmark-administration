...
class BankReconciliationController extends Controller
{
    ...
    public function match(BankReconciliation $reconciliation)
    {
        ...
        $systemPayments = ProjectPayment::with(['project'])
            ->where('bank_account_id', $reconciliation->cash_account_id)
            ->whereBetween('payment_date', [$reconciliation->start_date, $reconciliation->end_date])
            ->where('is_reconciled', false)
            ->get();

        $systemExpenses = ProjectExpense::with(['project'])
            ->where('bank_account_id', $reconciliation->cash_account_id)
            ->whereBetween('expense_date', [$reconciliation->start_date, $reconciliation->end_date])
            ->where('is_reconciled', false)
            ->get();

        $systemInvoicePayments = PaymentSchedule::with(['invoice.project'])
            ->where('status', 'paid')
    ...
    private function importBCAFormat(BankReconciliation $reconciliation, $handle)
    {
        ...
        Log::debug("Detected BCA CSV format for reconciliation #{$reconciliation->id}");
        ...
        while (($line = fgets($handle)) !== false) {
            ...
            if (stripos($row[0], 'Account No') !== false ||
                ...
                Log::debug("Row {$rowNumber}: Header row - {$row[0]}");
            ...
            if (stripos($row[0], 'Date') !== false &&
                ...
                $inDataSection = true;
                Log::debug("Row {$rowNumber}: Data section header detected");
            ...
            if (stripos($row[0], 'Starting Balance') !== false ||
                ...
                Log::debug("Row {$rowNumber}: Footer detected - {$row[0]}");
            ...
            if ($inDataSection && count($row) >= 6) {
                try {
                    ...
                    if ($amount == 0) {
                        Log::debug("Skipping row {$rowNumber}: Zero amount");
                    ...
                    Log::debug("BCA Row {$rowNumber} parsed: Date={$transactionDate}, Debit={$debitAmount}, Credit={$creditAmount}, Balance={$runningBalance}");
            ...
            try {
                ...
                if (count($row) >= 5) {
                    ...
                    if ($debitAmount == 0 && $creditAmount == 0 && $runningBalance == 0) {
                        Log::debug("Skipping row {$rowNumber}: All amounts are zero");
                    ...
                    Log::debug("Row {$rowNumber} parsed: Date={$transactionDate}, Debit={$debitAmount}, Credit={$creditAmount}");
                }
            } catch (\Exception $e) {
